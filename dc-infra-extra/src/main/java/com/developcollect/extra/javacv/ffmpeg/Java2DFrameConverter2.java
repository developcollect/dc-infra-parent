package com.developcollect.extra.javacv.ffmpeg;
 ...
 * @author Samuel Audet
 */
public class Java2DFrameConverter2 extends Java2DFrameConverter {
    ...
    public static final byte[]
            ...
            gamma18 = new byte[256],
            gamma18inv = new byte[256];

    static {
        for (int i = 0; i < 256; i++) {
            gamma22[i] = (byte) Math.round(Math.pow(i / 255.0, 2.2) * 255.0);
            gamma22inv[i] = (byte) Math.round(Math.pow(i / 255.0, 1 / 2.2) * 255.0);
            gamma18[i] = (byte) Math.round(Math.pow(i / 255.0, 1.8) * 255.0);
            gamma18inv[i] = (byte) Math.round(Math.pow(i / 255.0, 1 / 1.8) * 255.0);
        }
    }
