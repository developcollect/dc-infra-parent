...
import com.developcollect.core.utils.FileNameUtil;
import com.developcollect.core.utils.FileUtil;
import com.developcollect.core.utils.StrUtil;
import com.developcollect.extra.media.enums.CrfValueEnum;
import com.developcollect.extra.media.enums.PresetValueEnum;
import com.developcollect.extra.media.gif.AnimatedGifEncoder;
...
//    "F:\\videoUtil\\ffmpeg\\ffmpeg\\bin\\ffmpeg.exe"
    // "/usr/bin/ffmpeg";
    ...
    public static boolean setFFmpegPath(String ffmpeg_path) {
        if (StrUtil.isBlank(ffmpeg_path)) {
    ...
    public static boolean isExecutable() {
        ...
        if (StrUtil.isBlank(ffmpegVersionStr)) {
    ...
    public static String executeCommand(List<String> commonds) {
        if (CollectionUtil.isEmpty(commonds)) {
        ...
        try {
           ...
           /* // 取出输出流和错误流的信息
            ...
            String resultStr = StrUtil.isBlank(stringBuilder.toString()) ? "【异常】" : "正常";
...
//            FileUtils.copyInputStreamToFile(inputStream, file);
    ...
    private static String getFormat(File file) {
        String format = FileNameUtil.getSuffix(file);
        return format;
    }
     ...
     * @param impFile   图片保存路径
     */
    public static File cutVideoCover(File videoFile, String impFile) {
        return cutVideoCover(videoFile, impFile, 1000);
    }
     ...
     * @param videoFile 视频文件
     * @param impFile   图片保存路径
     * @param time      截取位置（视频的第几毫秒）
     */
    public static File cutVideoCover(File videoFile, String impFile, long time) {
        cutVideoFrame(videoFile, impFile, time, null, null, 0, false);
        return new File(impFile);
    }
...
////        cutVideoCover(outFile, "f:/videoTest/bbbhh.jpg");
    ...
    public static boolean isMusic(String filenameOrSuffix) {
        String suffix = StrUtil.subAfter("." + filenameOrSuffix, ".", true).toLowerCase();
        return ArrayUtil.contains(AUDIO_SUFFIX, suffix);
    }
    ...
    public static boolean isVideo(String filenameOrSuffix) {
        String suffix = StrUtil.subAfter("." + filenameOrSuffix, ".", true).toLowerCase();
        return ArrayUtil.contains(VIDEO_SUFFIX, suffix);
    }
}