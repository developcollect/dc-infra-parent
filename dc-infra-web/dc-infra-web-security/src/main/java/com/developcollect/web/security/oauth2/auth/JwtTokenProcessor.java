...
public class JwtTokenProcessor implements TokenProcessor {
     ...
     * @param key 加密和验签的秘钥
     */
    public JwtTokenProcessor(String key) {
        this(key, LambdaUtil::nop, LambdaUtil::nop);
    }

    public JwtTokenProcessor(String key, Consumer<JWT> accessTokenJwtConsumer, Consumer<JWT> refreshTokenJwtConsumer) {
        this(JWTSignerUtil.hs256(Base64.decode(key)), accessTokenJwtConsumer, refreshTokenJwtConsumer);
    }

    public JwtTokenProcessor(JWTSigner jwtSigner, Consumer<JWT> accessTokenJwtConsumer, Consumer<JWT> refreshTokenJwtConsumer) {
